...
import org.java.plus.dag.core.base.mvel2.ast.And;
import org.java.plus.dag.core.base.mvel2.ast.BinaryOperation;
import org.java.plus.dag.core.base.mvel2.ast.BooleanNode;
import org.java.plus.dag.core.base.mvel2.ast.Contains;
import org.java.plus.dag.core.base.mvel2.ast.Convertable;
import org.java.plus.dag.core.base.mvel2.ast.DeclTypedVarNode;
import org.java.plus.dag.core.base.mvel2.ast.Function;
import org.java.plus.dag.core.base.mvel2.ast.Instance;
import org.java.plus.dag.core.base.mvel2.ast.IntAdd;
import org.java.plus.dag.core.base.mvel2.ast.IntDiv;
import org.java.plus.dag.core.base.mvel2.ast.IntMult;
import org.java.plus.dag.core.base.mvel2.ast.IntOptimized;
import org.java.plus.dag.core.base.mvel2.ast.IntSub;
import org.java.plus.dag.core.base.mvel2.ast.LiteralNode;
import org.java.plus.dag.core.base.mvel2.ast.Or;
import org.java.plus.dag.core.base.mvel2.ast.RegExMatchNode;
import org.java.plus.dag.core.base.mvel2.ast.Soundslike;
import org.java.plus.dag.core.base.mvel2.ast.Strsim;
...
import static org.java.plus.dag.core.base.mvel2.util.ASTBinaryTree.buildTree;
import static org.java.plus.dag.core.base.mvel2.util.ParseTools.__resolveType;
import static org.java.plus.dag.core.base.mvel2.util.ParseTools.boxPrimitive;
...
public class CompilerTools {
     ...
     */
    public static ASTLinkedList finalizePayload(ASTLinkedList astLinkedList, boolean secondPassOptimization,
                                                ParserContext pCtx) {
        ...
        while (astLinkedList.hasMoreNodes()) {
            if ((tk = astLinkedList.nextNode()).getFields() == -1) {
                ...
                if ((tkOp = astLinkedList.nextNode()).getFields() == -1) {
                    ...
                    if (op == -1) {
                        throw new CompileException("illegal use of operator: " + tkOp.getName(), tkOp.getExpr(),
                            tk.getStart());
                    }
                    ...
                    while (astLinkedList.hasMoreNodes() && (tkOp2 = astLinkedList.nextNode()).isOperator()
                        ...
                        if (PTABLE[op2] > PTABLE[op]) {
                            //       bo.setRightMost(new BinaryOperation(op2, bo.getRightMost(), astLinkedList
                            // .nextNode(), pCtx));
                    ...
                    optimizedAst.addTokenNode(
                        new DeclTypedVarNode(tkOp.getName(), tkOp.getExpr(), tkOp.getStart(), tk.getOffset(),
                    ...
                    if ((tkOp = astLinkedList.nextNode()).getFields() == -1) {
                        ...
                        while (astLinkedList.hasMoreNodes() && (tkOp2 = astLinkedList.nextNode()).isOperator()
                            ...
                            if ((tkOp = tkOp2).getOperator() == Operator.AND) {
                                bool.setRightMost(
                                    new And(bool.getRightMost(), astLinkedList.nextNode(), pCtx.isStrongTyping(),
                                        pCtx));
                            } else {
            ...
            case TERNARY:
                if (pCtx.isStrongTyping() && tk.getEgressType() != Boolean.class
                    ...
                    throw new RuntimeException(
                        "Condition of ternary operator is not of type boolean. Found " + tk.getEgressType());
                }
    ...
    public static void expectType(ParserContext pCtx, Accessor expression, Class type, boolean compileMode) {
        ...
        } else if (retType == null || !Object.class.equals(retType) && !boxPrimitive(type).isAssignableFrom(
            boxPrimitive(retType))) {
            throw new CompileException("was expecting type: " + type.getName() + "; but found type: "
                + (retType != null ? retType.getName() : "<Unknown>"), new char[0], 0);
        }
    }
    ...
    public static Class getReturnTypeFromOp(int operation, Class left, Class right) {
        switch (operation) {
            ...
            case Operator.MOD:
            case Operator.DIV:
                if (left == Object.class || right == Object.class) { return Object.class; } else {
    ...
    public static Accessor extractAccessor(ASTNode n) {
        if (n instanceof LiteralNode) { return new ExecutableLiteral(n.getLiteralValue()); } else {
    ...
    public static Number signNumber(Object number) {
        if (number instanceof Integer) {
            ...
            throw new CompileException("expected a numeric type but found: " + number.getClass().getName(), new char[0],
                0);
        }
    }
