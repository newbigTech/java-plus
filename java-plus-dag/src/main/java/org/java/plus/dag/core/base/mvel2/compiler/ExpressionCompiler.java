...
import org.java.plus.dag.core.base.mvel2.ast.Assignment;
...
import org.java.plus.dag.core.base.mvel2.util.ASTLinkedList;
import org.java.plus.dag.core.base.mvel2.util.CompilerTools;
...
import static org.java.plus.dag.core.base.mvel2.ast.ASTNode.COMPILE_IMMEDIATE;
import static org.java.plus.dag.core.base.mvel2.ast.ASTNode.OPT_SUBTR;
import static org.java.plus.dag.core.base.mvel2.util.CompilerTools.finalizePayload;
import static org.java.plus.dag.core.base.mvel2.util.CompilerTools.signNumber;
import static org.java.plus.dag.core.base.mvel2.util.ParseTools.subCompileExpression;
import static org.java.plus.dag.core.base.mvel2.util.ParseTools.unboxPrimitive;
            ...
            if (pCtx.isFatalError()) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (e.getExpr() != expr) {
                        ...
                        err.append("\n - ").append("(").append(e.getLineNumber()).append(",").append(e.getColumn())
                            .append(")")
        ...
        try {
            ...
            while ((tk = nextToken()) != null) {
                ...
                if (tk instanceof Substatement) {
                    ...
                    if (compiled == null) {
                        ExpressionCompiler subCompiler = new ExpressionCompiler(expr, tk.getStart(), tk.getOffset(),
                            pCtx);
                ...
                if (!verifyOnly && tk.isLiteral()) {
                    ...
                    if ((tkOp = nextTokenSkipSymbols()) != null && tkOp.isOperator()
                        ...
                        /**
                         * If the next token is ALSO a literal, then we have a candidate for a compileShared-time
                         * literal
                         * reduction.
                         */
                            ...
                            while ((tkOp2 = nextTokenSkipSymbols()) != null) {
                                if (isBooleanOperator(tkOp2.getOperator())) {
                                    ...
                                    if (tkLA2.isLiteral()) {
                                        ...
                                        astBuild.addTokenNode(new OperatorNode(tkOp2.getOperator(), expr, st, pCtx),
                                            verify(pCtx, tkLA2));
                                        break;
                                    }
            ...
            if (e instanceof RuntimeException) { throw (RuntimeException)e; } else {
                throw new CompileException(e.getMessage(), expr, st, e);
            }
        }
    }
                    ...
                    if (pCtx.isStrictTypeEnforcement()) {
                        ...
                        if (!returnType.isAssignableFrom(c.getKnownEgressType()) && c.isLiteralOnly()) {
                            if (canConvert(c.getKnownEgressType(), returnType)) {
                                ...
                                try {
                                    a.setValueStatement(
                                        new ExecutableLiteral(convert(c.getValue(null, null), returnType)));
                                    return tk;
                                } catch (Exception e) {
                                    // fall through.
                                }
