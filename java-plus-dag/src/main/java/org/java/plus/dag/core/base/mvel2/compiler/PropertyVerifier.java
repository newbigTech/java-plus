...
public class PropertyVerifier extends AbstractOptimizer {
    ...
    private Class getBeanProperty(Class ctx, String property) {
        ...
        if (pCtx != null && pCtx.getParserConfiguration() != null ? pCtx.getParserConfiguration().isAllowNakedMethCall()
    ...
    private Class getCollectionProperty(Class ctx, String property) {
        ...
        if (pCtx.isStrictTypeEnforcement()) {
            if (Map.class.isAssignableFrom(property.length() != 0 ? ctx = getBeanProperty(ctx, property) : ctx)) {
                ctx = type2Class(pCtx.getLastTypeParameters() != null && pCtx.getLastTypeParameters().length != 0 ? pCtx
                    .getLastTypeParameters()[1] : Object.class);
            } else if (Collection.class.isAssignableFrom(ctx)) {
                ctx = pCtx.getLastTypeParameters() == null || pCtx.getLastTypeParameters().length == 0 ? Object.class
                ...
                if (errors.size() < pCtx.getErrorList().size()) {
                    for (ErrorDetail detail : pCtx.getErrorList()) {
                        if (!errors.contains(detail)) {
                            detail.setExpr(expr);
                            detail.setCursor(new String(expr).substring(this.st).indexOf(new String(subtokens.get(i)))
                                + this.st);
                            detail.setColumn(0);
