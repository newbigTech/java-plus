 ...
 */
public class SplitShow extends Operation<DataSet<Row>> {
    private Operation<DataSet<Row>> input;

    private SplitShow() {}

    public SplitShow(Operation<DataSet<Row>> input) {
        this.input = input;
        depend(input);
    }
    ...
    public DataSet<Row> apply(ProcessorContext ctx) {
        ...
        input.get().getData().forEach(row -> {
            if (row.getFieldValue(AllFieldName.type).equals("2"))  { result.getData().add(row); }
        });
        return result;
    }
}