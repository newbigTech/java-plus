...
public class CollectionParser {
    ...
    public Object parseCollection(char[] property, int start, int offset, boolean subcompile, Class colType,
                                  ParserContext pCtx) {
    ...
    private Object parseCollection(boolean subcompile) {
        if (end - start == 0) {
            if (type == LIST) { return new ArrayList(); } else { return EMPTY_ARRAY; }
        }
            ...
            switch (property[cursor]) {
                ...
                case '[':
                    ...
                    Object o = new CollectionParser(newType).parseCollection(property, (st = cursor) + 1,
                        (cursor = balancedCapture(property, st, end, property[st])) - st - 1, subcompile, colType,
                        pCtx);
    ...
    private void subCompile(int start, int offset) {
        if (colType == null) {
            ...
            if (r != null && !isAssignableFrom(colType, r) && (isStrongType() || !DataConversion.canConvert(r,
                colType))) {
