...
import static org.java.plus.dag.core.base.mvel2.PropertyAccessor.set;
...
public class DeepAssignmentNode extends ASTNode implements Assignment {
    ...
    public DeepAssignmentNode(char[] expr, int start, int offset, int fields, int operation, String name,
                              ParserContext pCtx) {
        super(pCtx);
        ...
        if (operation != -1) {
            this.egressType = ((statement =
                (ExecutableStatement)subCompileExpression(
                    createShortFormOperativeAssignment(this.property = name, expr, start, offset, operation), pCtx)))
                .getKnownEgressType();
