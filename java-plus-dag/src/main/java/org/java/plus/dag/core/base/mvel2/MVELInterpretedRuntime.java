...
import static org.java.plus.dag.core.base.mvel2.Operator.NOOP;
...
public class MVELInterpretedRuntime extends AbstractParser {
                    ...
                    case OP_OVERFLOW:
                        if (!tk.isOperator()) {
                            if (!(stk.peek() instanceof Class)) {
                                throw new CompileException("unexpected token or unknown identifier:" + tk.getName(),
                                    expr, st);
                            }
    ...
    private boolean unwindStatement(int operator) {
        ...
        switch (operator) {
            case AND:
                while ((tk = nextToken()) != null && !tk.isOperator(Operator.END_OF_STMT) && !tk.isOperator(
                    Operator.OR)) {
                    //nothing
                }
                break;
    ...
    public MVELInterpretedRuntime(char[] expr, int start, int offset, Object ctx,
    ...
    public MVELInterpretedRuntime(char[] expr, int start, int offset, Object ctx,
                                  VariableResolverFactory resolverFactory, ParserContext pCtx) {
        super(pCtx);
        this.expr = expr;
        this.start = start;
        this.end = start + offset;
