...
public class PropertyTools {
    ...
    public static Method getGetter(Class clazz, String property) {
        ...
        for (Method meth : clazz.getMethods()) {
            if ((meth.getModifiers() & PUBLIC) != 0 && (meth.getModifiers() & STATIC) == 0
                ...
                && (getter.equals(meth.getName()) || property.equals(meth.getName()) || ((isGet.equals(meth.getName())
                || simpleIsGet.equals(meth.getName())) && meth.getReturnType() == boolean.class)
    ...
    public static boolean contains(Object toCompare, Object testValue) {
        if (toCompare == null) { return false; } else if (toCompare instanceof String) {
        ...
        } else if (toCompare instanceof Map) { return ((Map)toCompare).containsKey(testValue); } else if (toCompare
            .getClass().isArray()) {
            for (Object o : ((Object[])toCompare)) {
                if (testValue == null && o == null) { return true; } else if (o != null && o.equals(testValue)) {
                    return true;
                }
            }
        }
    ...
    public static boolean isAssignable(Class to, Class from) {
        return (to.isPrimitive() ? boxPrimitive(to) : to).isAssignableFrom(
            from.isPrimitive() ? boxPrimitive(from) : from);
    }
