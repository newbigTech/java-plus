...
import org.java.plus.dag.core.base.mvel2.PropertyAccessor;
...
import org.java.plus.dag.core.base.mvel2.optimizers.OptimizerFactory;
...
import static org.java.plus.dag.core.base.mvel2.MVEL.analyze;
import static org.java.plus.dag.core.base.mvel2.MVEL.eval;
import static org.java.plus.dag.core.base.mvel2.optimizers.OptimizerFactory
    .getThreadAccessorOptimizer;
import static org.java.plus.dag.core.base.mvel2.util.ArrayTools.findFirst;
import static org.java.plus.dag.core.base.mvel2.util.CompilerTools
    .getInjectedImports;
...
import static org.java.plus.dag.core.base.mvel2.util.ParseTools.subArray;
            ...
            if (pCtx != null && pCtx.hasImport(typeDescr.getClassName())) {
                ...
                try {
                    ...
                    if (pCtx.isStrongTyping()) {
                        pCtx.addError(
                            new ErrorDetail(expr, start, true, "could not resolve class: " + typeDescr.getClassName()));
                    }
                    return;
                    // do nothing.
                }
    ...
    private void rewriteClassReferenceToFQCN(int fields) {
        ...
        if (typeDescr.getClassName().indexOf('.') == -1) {
            ...
            if (idx == -1) {
                ...
                for (int i = 0; i < idx; i++) { this.name[i] = fqcn[i]; }
            } else {
                char[] newName = new char[fqcn.length + (name.length - idx)];

                for (int i = 0; i < fqcn.length; i++) { newName[i] = fqcn[i]; }

                int i0 = name.length - idx;
                int i1 = fqcn.length;
                for (int i = 0; i < i0; i++) { newName[i + i1] = name[i + idx]; }
        ...
        if (newObjectOptimizer == null) {
            ...
            if (typeDescr.isArray()) {
                return (newObjectOptimizer = new NewObjectArray(getBaseComponentType(egressType.getComponentType()),
                    typeDescr.getCompiledArraySize()))
            ...
            try {
                ...
                newObjectOptimizer = optimizer.optimizeObjectCreation(pCtx, name, 0, name.length, ctx, thisValue,
                    factory);
        ...
        try {
            if (typeDescr.isArray()) {
                ...
                if (constructorParms != null) {
                    ...
                    Constructor<?> cns = Class.forName(typeDescr.getClassName(), true,
