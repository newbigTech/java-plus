...
public class ExecutionStack {
    ...
    public Object peek() {
        if (size == 0) { return null; } else { return element.value; }
    }
    ...
    public Boolean popBoolean() {
        ...
        try {
            if (element.value instanceof Boolean) { return (Boolean)element.value; }
            throw new ScriptRuntimeException("expected Boolean; but found: " + (element.value == null ? "null"
                : element.value.getClass().getName()));
        } finally {
            element = element.next;
            assert size == deepCount();
