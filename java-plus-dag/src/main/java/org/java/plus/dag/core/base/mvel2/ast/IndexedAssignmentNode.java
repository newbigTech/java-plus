    ...
    public IndexedAssignmentNode(char[] expr, int start, int offset, int fields, int operation,
        ...
        if (operation != -1) {
            ...
            this.egressType = (statement = (ExecutableStatement)
                subCompileExpression(stmt = createShortFormOperativeAssignment(name, expr, start, offset, operation),
            ...
            if (col = ((endOfName = (short)findFirst('[', 0, this.name.length(), indexTarget = this.name.toCharArray()))
                > 0)) {
