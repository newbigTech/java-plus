    ...
    public Class<?> getReturnType(boolean strongTyping) {
        ...
        switch (((OperatorNode)root).getOperator()) {
            ...
            case DIV:
                if (strongTyping && !CompatibilityStrategy.areEqualityCompatible(leftType, rightType)) {
                    throw new RuntimeException(
                        "Associative operation requires compatible types. Found " + leftType + " and " + rightType);
                }
                return Double.class;
            case TERNARY_ELSE:
                if (strongTyping && !CompatibilityStrategy.areEqualityCompatible(leftType, rightType)) {
            ...
            case NEQUAL:
                if (strongTyping && !CompatibilityStrategy.areEqualityCompatible(leftType, rightType)) {
                    throw new RuntimeException(
                        "Comparison operation requires compatible types. Found " + leftType + " and " + rightType);
                }
                return Boolean.class;
            case LTHAN:
            case LETHAN:
